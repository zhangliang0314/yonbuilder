...
import { Tabs, Button, Popover, Checkbox } from 'antd'
...
// import ReactEcharts from '@mdf/metaui-web/lib/components/AsyncComponents/AsyncEchartsForReact'
...
class SaleTrend extends React.Component {
  constructor (props) {
    ...
    const defaultShops = props.showStore === false ? [] : [{
  ...
  getOption () {
    const legendData = []; const seriesData = [];
    const data = this.props.home.saleTrendData || [];
    const xAxisData = data.length > 0 ? data[0].dates : [];
    data.forEach(element => {
      const ele = {}; const item = {
        type: 'line',
      ...
      const accuracy = cb.rest.AppContext.option.amountofdecimal// 金额
  ...
  getCardContent () {
    const shops = this.props.home.saleTrend_shops || [];
    const allShopsData = this.props.user.showStore === false ? [] : (this.props.user.userStores || []);
    const getPopShops = (shops, currentShop) => {
      const isChecked = shops.filter(ele => {
      ...
      <div className='home-title-right'>
        ...
        <div className='home-title-date'>
          <Tabs onTabClick={(e) => this.onTabClick(e)} defaultActiveKey='seven' size='small'>
            <TabPane tab='近7天' key='seven' />
            <TabPane tab='近30天' key='thirty' />
            <TabPane tab='近90天' key='ninety' />
          </Tabs>
        </div>
      </div>
    </div>)
  ...
  onTabClick (e) {
    const endDate = common.getDate(e, '0');
    const startDate = common.getDate(e, '1');
    // let lastStartDate = common.getDate('last' + e, '0');
    // let lastEndDate = common.getDate('last' + e, '1');
    const storeIdArr = this.props.home.storeIdArr;
  ...
  popoverButtonClick (type) {
    ...
    const shops = this.props.home.saleTrend_shops;
    const copyShops = this.props.home.saleTrend_copyShops;
    if (type == 'save') {
      // this.actions().setOptions({ trend_showData: shops })
      // this.setState({ copyShops: shops })
      const { saleTrend_beginData, saleTrend_endData } = this.props.home
      const now = new Date()
      const beginDate = saleTrend_beginData || new Date(now.getTime() - 1000 * 60 * 60 * 24 * 6).format('yyyy-MM-dd');
      const endDate = saleTrend_endData || now.format('yyyy-MM-dd');
      const storeIdArr = [];
      shops.forEach(ele => {
