...
class HomeControl extends React.Component {
  ...
  getControlByType (ele) {
    const cControlType = ele.cControlType;
    let control;
    switch (cControlType) {
      case 'SaleRank':
  ...
  getRowControl (row) {
    ...
    row.forEach(function (ele) {
      const colControl = this.getColControl(ele.col);
  ...
  getColControl (col) {
    ...
    col.forEach(function (ele) {
      ...
      if (ele.row) {
        const row = this.getRowControl(ele.row);
        colControl.push(<Col span={ele.span}>{row}</Col>);
      } else {
        const control = this.getControlByType(ele);
  ...
  getControlByLayout (layoutJson) {
    const row = layoutJson.row;
    const rowControl = this.getRowControl(row);
    return rowControl;
  }

  render () {
    const { layOut } = this.props.home;
    if (!layOut)
      return null;
